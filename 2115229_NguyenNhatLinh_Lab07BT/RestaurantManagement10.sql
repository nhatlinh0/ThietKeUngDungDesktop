Create database [RestaurantManagement10]
USE [RestaurantManagement10]
GO
...
select * from [Table]
INSERT [dbo].[Table] ([ID], [Name], [Status], [Capacity]) VALUES (1009, N'VIP.3', 0, 10)
...
as
begin
	INSERT [dbo].[Category] ([Name], [Type]) VALUES ( @Name, @Type)
end
...
create procedure ShowBillsWithDate
@date1 datetime,
@date2 datetime
as
begin
	select * from Bills where Bills.CheckoutDate Between @date1 and @date2
end

create procedure ShowBillDetails 
@id int
as
begin
	select * from BillDetails where BillDetails.InvoiceID = @id;
end

create proc ShowAccount 
as
begin
	select * from Account   
end
...
as
begin
	INSERT [dbo].[Account] ([AccountName], [Password], [FullName], [Email], [Tell], [DateCreated]) VALUES (@accountname, @password, @fullName, @email, @tell, @date)
end
...
as
begin
	update Account set [Password] = @password, FullName = @fullName, Email = @email, Tell = @tell, DateCreated = @datet where AccountName = @accountname
end

exec UpdateAccount N'lgcon', N'qweqw', N'le gia cong', N'asda', N'asd','14/04/2021'
select * from Account
create proc ResetAccount 
@accountname nvarchar(100)
as
begin 
	update Account set [Password] = N'123' where AccountName = @accountname
end
